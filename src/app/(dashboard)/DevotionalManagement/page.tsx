    ...
    const handleDelete = async (id: string) => {
        try {
            ...
            if (result.isConfirmed) {
                setDevotionals((prev) => prev.filter((devotional) => devotional._id !== id));
    ...
    const handleStatusChange = (id: string, value: string) => {
        setDevotionals((prevDevotionals) =>
            prevDevotionals.map((devotional) =>
                devotional._id === id ? { ...devotional, status: value === 'active' } : devotional
            )
        );
    };
            ...
            <div className="w-full overflow-x-auto">
                <table className="min-w-[700px] w-full overflow-hidden shadow border-separate border-spacing-x-4">
                    <thead className="text-[#794A3A] font-dm-sans text-[14px] font-semibold ">
                        <tr>
                            <th className="p-[5px] border-b border-[#505050]  text-center   w-fit">S.No:</th>
                            <th className="p-[5px] border-b border-[#505050]  text-center   w-fit">Title</th>
                            <th className="p-[5px] border-b border-[#505050]  text-center   w-fit">Scripture</th>
                            <th className="p-[5px] border-b border-[#505050]  text-center   w-fit">Status</th>
                            <th className="p-[5px] border-b border-[#505050]  text-center   w-fit">Created At</th>
                            <th className="p-[5px] border-b border-[#505050]  text-center   w-fit">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="dark:bg-gray-900 ">
                        {devotionals.length === 0 ? (
                            ...
                            paginatedDevotionals.map((devotional, i) => (
                                <tr key={devotional._id} className="border-b border-[#DEE2E6]/50 ">
                                    <td className="p-[5px] text-center border-b-1 border-[#F9F9F9] text-[#5B5B5B]">{(currentPage - 1) * ITEMS_PER_PAGE + i + 1}</td>
                                    <td className="p-[5px] text-center border-b-1 border-[#F9F9F9] text-[#5B5B5B]">{devotional.title}</td>
                                    <td className="p-[5px] text-center border-b-1 border-[#F9F9F9] text-[#5B5B5B]">{devotional.scripture}</td>
                                    <td className="p-[5px] text-center border-b-1 border-[#F9F9F9] text-[#5B5B5B]">
                                        <select
                                            value={devotional.status ? 'active' : 'inactive'}
                                            onChange={(e) => handleStatusChange(devotional._id, e.target.value)}
                                            className={`p-[6px] font-medium focus:outline-none transition-colors rounded-[6px] cursor-pointer duration-200 ${getBgClass(devotional.status)}`}
                                        >
...
<path d="M5.33333 14.6667H6.28333L12.8 8.15L11.85 7.2L5.33333 13.7167V14.6667ZM4.66667 16C4.47778 16 4.31956 15.936 4.192 15.808C4.06444 15.68 4.00044 15.5218 4 15.3333V13.7167C4 13.5389 4.03333 13.3693 4.1 13.208C4.16667 13.0467 4.26111 12.9051 4.38333 12.7833L12.8 4.38333C12.9333 4.26111 13.0807 4.16667 13.242 4.1C13.4033 4.03333 13.5727 4 13.75 4C13.9273 4 14.0996 4.03333 14.2667 4.1C14.4338 4.16667 14.5782 4.26667 14.7 4.4L15.6167 5.33333C15.75 5.45556 15.8471 5.6 15.908 5.76667C15.9689 5.93333 15.9996 6.1 16 6.26667C16 6.44444 15.9693 6.614 15.908 6.77533C15.8467 6.93667 15.7496 7.08378 15.6167 7.21667L7.21667 15.6167C7.09444 15.7389 6.95267 15.8333 6.79133 15.9C6.63 15.9667 6.46067 16 6.28333 16H4.66667ZM12.3167 7.68333L11.85 7.2L12.8 8.15L12.3167 7.68333Z" fill="white"/>
</svg>
